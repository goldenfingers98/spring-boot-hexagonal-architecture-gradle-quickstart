package tn.ksoftwares.hexagonal.domain.core.advice;
...
public class UserValidationAOP {
    ...
    @Around("execution(* tn.ksoftwares.hexagonal.domain.core.service.UserServiceImpl.saveUser(..)) && args(tn.ksoftwares.hexagonal.domain.model.pojo.User))")
    ...
    private void validateUser(User user) throws IllegalArgumentException, IllegalAccessException, DomainConstraintViolationException, MalformedFieldException {
        ...
        if (!violations.isEmpty()) {
            logger.error("User instance violates constraints.");
